...
export async function GET(request: NextRequest) {
  ...
  const data = await op_client.query(
...
// console.log('fromAddress:',fromAddress);
 ...
 await fetchVotingPower(addresses).then((votingPowers) => {
    votingPowersGlobal = votingPowers.map((vp: string) => (Number(vp) / 10**18).toString()); // Assign the result to the global variable
        ...
        dates.map(async (date) => {
          ...
          const addressesToUpdate = [
            {   
                ...
                newVotingPower: (Number(votingPowersGlobal[1])-Number(votingPowersGlobal[0])).toString() // New voting power
            },
            {
                address: toAddress.toLowerCase(), // To address (delegate recipient)
                newVotingPower: (Number(votingPowersGlobal[2])+Number(votingPowersGlobal[0])).toString() // New voting power
            }
          ];
          // Update voting power for specific addresses
          data = data.map((delegate:any) => {
            const updateMatch = addressesToUpdate.find(
              update => update.address.toLowerCase() === delegate.delegate_id.toLowerCase()
            );
...
//   console.log('data:', data);   
          ...
          const totalVotingPower = data.reduce(
            (sum: number, delegate: any) => {
